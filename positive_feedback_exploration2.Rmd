...
count = 0;
data <- matrix(0,nrow=round(maxtime/timestep)+1,ncol=1);
...
```{r plot 4}
library(fields);
lts = maxtime/timestep;
step = 1000;
count = 0;
count2 = 0;
F <- matrix(0,nrow=6,ncol=round(lts/step));
for (a in seq(1,lts,by=step)){
  count2 = count2 + 1;
for (k_3 in seq(0,0.005,by=0.001)){
  count = count + 1;
  output = data[1:a,count]
  temp_mean_A = mean(output)
  temp_var_A = var(output)
  F[count,count2] = temp_var_A/temp_mean_A;
}
  count = 0;
  }
k_3 = seq(0,0.005,by=0.001);
...
for (k_3 in seq(0,0.005,by=0.001)){
  count1 = count1 + 1;
  entropy_A[count1] = entropy(data[,count1]);
}
k_3 = seq(0,0.005,by=0.001);
plot(k_3,entropy_A);
```
