...
Let's define parameters, run 100 trajectories of the model and plot the output:
```{r parameters}
k_1 = 10;
k_2 = 0.01;
k_3 = 0.001;
k_steady = k_1/(k_2-k_3);
maxtime = 300;
timestep = 25;
numberofrealisations = 100;
#output = simple_positive_feedback(k_1,k_2,k_3,maxtime,timestep,numberofrealisations,round(runif(1,min=.2,max=.5)*10));
#plot(seq(0,maxtime,timestep),t(rowMeans(output)));
count = 0;
mu_A <- rep(0, 6);
leg <- matrix(0,1,ncol=length(seq(0.006,0,by=-0.002)))
for (k_3 in seq(0.009,0,by=-0.003)){
  count = count + 1;
  output = simple_positive_feedback(k_1,k_2,k_3,maxtime,timestep,numberofrealisations,round(runif(1,min=.3,max=.5)*1000));
  temp_mean_A = rowMeans(output);
  mu_A[count] = temp_mean_A[length(temp_mean_A)];

  if (count==1){plot(seq(0,maxtime,timestep),t(rowMeans(output)),type='b',pch=19,col=count,ylim=c(0,3000),xlab = 'time', ylab='[A]',main='plot')
     }
{lines(seq(0,maxtime,timestep),t(rowMeans(output)),type='b',pch=19,col=count)
 }
leg[count]<-k_3
}
legend("topleft",c(paste('k_3=',leg[1]),paste('k_3=',leg[2]),paste('k_3=',leg[3]),paste('k_3=',leg[4])),pch=19,col=1:count)
```
...
count = 0;
mu_A <- rep(0, 6);
for (k_3 in seq(0.005,0,by=-0.001)){
  count = count + 1;
  output = simple_positive_feedback(k_1,k_2,k_3,maxtime,timestep,numberofrealisations,round(runif(1,min=.3,max=.5)*1000));
  temp_mean_A = rowMeans(output);
  mu_A[count] = temp_mean_A[length(temp_mean_A)];
...
for (k_3 in seq(0.009,0,by=-0.003)){
  ...
  output = simple_positive_feedback(k_1,k_2,k_3,maxtime,timestep,1,round(runif(1,min=.3,max=.5)*1000));
  ...
  if (count1==1){
  plot(seq(0,maxlag,by=1),colMeans(autocorr),type="b",pch=19,col=count1)
  }
  else{
lines(seq(0,maxlag,by=1),colMeans(autocorr),type="b",pch=19,col=count1)
}
#lines(lag,autocorrelation,col="green")
}
legend("topright",c(paste('k_3=',leg[1]),paste('k_3=',leg[2]),paste('k_3=',leg[3]),paste('k_3=',leg[4])),pch=19,col=1:count)
