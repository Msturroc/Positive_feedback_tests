...
for (k_3 in seq(0,0.005,by=0.001)){
  count = count + 1;
  output = simple_positive_feedback(k_1,k_2,k_3,maxtime*10000,timestep,numberofrealisations,round(k_1/(k_2-k_3)));
  temp_mean_A = mean(output);
  temp_var_A = var(output);
  F[count] = temp_var_A/temp_mean_A;
}
k_3 = seq(0,0.005,by=0.001);
plot(k_3,F);
```
...
for (k_3 in seq(0,0.005,by=0.001)){
  count1 = count1 + 1;
  output = simple_positive_feedback(1,0.01,k_3,10000,0.1,1,round(k_1/(k_2-k_3)));
  entropy_A[count1] = entropy(output);
}
k_3 = seq(0,0.005,by=0.001);
plot(k_3,entropy_A);
```
